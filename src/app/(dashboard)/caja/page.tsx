...
import { Wallet, Plus, TrendingUp, TrendingDown, DollarSign, CreditCard, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-0.5">
            Arqueo, desglose por métodos de pago e ingresos/egresos en tiempo real
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-4 flex items-center gap-3 border-l-4 border-l-[var(--color-wood)]">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-[var(--text-primary)]">${saldoCaja.toLocaleString()}</p>
          </div>
        </Card>

        <Card className="p-4 flex items-center gap-3 border-l-4 border-l-[var(--color-success)]">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-[var(--text-primary)]">${totalIngresos.toLocaleString()}</p>
          </div>
        </Card>

        <Card className="p-4 flex items-center gap-3 border-l-4 border-l-[var(--color-danger)]">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-[var(--text-primary)]">${totalEgresos.toLocaleString()}</p>
          </div>
        </Card>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          <Spinner size="lg" />
          <p className="text-xs text-[var(--text-muted)]">Cargando movimientos de caja...</p>
        </div>
        ...
        <Card className="p-12 text-center flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-xs text-[var(--text-muted)]">
            Ingresa un gasto o cobro para actualizar el saldo.
          </p>
        </Card>
      ) : (
              ...
              <tbody className="divide-y divide-[var(--border-default)] text-[var(--text-primary)]">
                {movimientos.map((mov) => (
                  <tr key={mov.id} className="hover:bg-[var(--bg-tertiary)]/50 transition-colors">
                    ...
                    <td className="p-3.5 font-medium">{mov.concepto}</td>
                    <td className="p-3.5 capitalize">{mov.metodo_pago.replace('_', ' ')}</td>
                    <td className={`p-3.5 font-bold ${mov.tipo === 'INGRESO' ? 'text-[var(--color-wood)]' : 'text-[var(--color-danger)]'}`}>
      ...
      <Modal
        ...
        description="Agrega un movimiento manual para ajustar la caja diaria"
        size="md"
      >
        <form onSubmit={handleCreateMovimiento} className="flex flex-col gap-4">
          <Input
            label="Concepto *"
            placeholder={tipo === 'INGRESO' ? 'Ej. Pago extraordinario clase suelta' : 'Ej. Compra de insumos / limpieza'}
            value={concepto}
